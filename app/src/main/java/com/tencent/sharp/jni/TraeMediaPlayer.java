...
public class TraeMediaPlayer implements MediaPlayer.OnCompletionListener,
    ...
    public boolean playRing(int datasource, int rsid, Uri res, String strFilePath, boolean loop, int loopCount, boolean ringMode, boolean hasCall, int callStreamType) {
        if (QLog.isColorLevel())
            QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | playRing datasource:" + datasource + " rsid:" + rsid + " uri:" + res + " filepath:" + strFilePath + " loop:" + (loop ? "Y" : "N") + " :loopCount" + loopCount + " ringMode:" + (ringMode ? "Y" : "N") + " hasCall:" + hasCall + " cst:" + callStreamType);

        if (loop == false && loopCount <= 0) {
            if (QLog.isColorLevel())
                QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | playRing err datasource:" + datasource + " loop:" + (loop ? "Y" : "N") + " :loopCount" + loopCount);
            return false;
        }
        try {
            try {
                ...
                switch (datasource) {
                    case TRAE_MEDIAPLAER_DATASOURCE_RSID:
                        ...
                        if (afd == null) {
                            if (QLog.isColorLevel())
                                QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | afd == null rsid:"
                                        + rsid);
                            mMediaPlay.release();
                            mMediaPlay = null;
                            return false;
                    ...
                    case TRAE_MEDIAPLAER_DATASOURCE_URI:
                        if (QLog.isColorLevel())
                            QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | uri:" + res);
                        mMediaPlay.setDataSource(_context, res);
                        break;
                    case TRAE_MEDIAPLAER_DATASOURCE_FILEPATH:
                        if (QLog.isColorLevel())
                            QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | FilePath:"
                                    + strFilePath);
                        mMediaPlay.setDataSource(strFilePath);
                        break;
                    default:
                        if (QLog.isColorLevel())
                            QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | err datasource:"
                                    + datasource);
                        mMediaPlay.release();
                        mMediaPlay = null;
                        break;
                ...
                if (_durationMS > 0) {
                    ...
                    _watchTimertask = new TimerTask() {
                        public void run() {
                            if (mMediaPlay != null) {
                                if (QLog.isColorLevel())
                                    QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | play timeout");
                                if (null != mCallback) {
                ...
                if (QLog.isColorLevel())
                    QLog.e("TRAE", QLog.CLR, "TraeMediaPlay | DurationMS:" + mMediaPlay.getDuration() + " loop:" + loop);
                ...
                if (QLog.isColorLevel())
                    QLog.d("TRAE", QLog.CLR, "TraeMediaPlay | IllegalStateException: "
                ...
                if (QLog.isColorLevel())
                    QLog.d("TRAE", QLog.CLR, "TraeMediaPlay | IllegalArgumentException: "
                            + ex.getLocalizedMessage() + " "
                            + ex.getMessage());

            } catch (SecurityException ex) {
                if (QLog.isColorLevel())
                    QLog.d("TRAE", QLog.CLR, "TraeMediaPlay | SecurityException: "
                            + ex.getLocalizedMessage() + " "
                            + ex.getMessage());
    ...
    private void volumeDo() {
        ...
        try {
            ...
            if (QLog.isColorLevel())
                QLog.e("TRAE", QLog.CLR, "TraeMediaPlay volumeDo currV:" + currV
    ...
    private void volumeUndo() {
        ...
        try {
            if (QLog.isColorLevel())
                QLog.e("TRAE", QLog.CLR, "TraeMediaPlay volumeUndo _prevVolume:" + _prevVolume);
            AudioManager am = (AudioManager) _context
                    .getSystemService(Context.AUDIO_SERVICE);
