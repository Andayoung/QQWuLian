...
public class VideoChatActivityHW extends Activity implements Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (VideoController.isHardwareDecoderEnabled()) {
            initPeerTextureView();
            VideoController.getInstance().enableVideoRecvNotify(true);
        } else {
            initPeerQQGlView();
        }
    ...
    protected void onPause() {
        ...
        if (VideoController.isHardwareDecoderEnabled()) {
            VideoController.getInstance().enableVideoRecvNotify(false);
        }
            ...
            if (intent.getAction().equalsIgnoreCase(VideoConstants.ACTION_STOP_VIDEO_CHAT)) {
                ...
                if (width != 0 && height != 0 && bitrate != 0 && fps != 0) {
                    if (encWidth != width || encHeight != height) {
                        encWidth = width;
                        encHeight = height;
                        Camera.Size shouldBeSize = choosePreviewSize(mCamera.getParameters(), width, height);
                        Camera.Size size = mCamera.getParameters().getPreviewSize();
                        //编码变大，需要重启摄像头采集更高分辨率。CAP在这里显示不准确。
                        if (size.width != shouldBeSize.width || size.height != shouldBeSize.height) {
                            closeCamera();
    ...
    public void openCamera(int encWidth, int encHeight) {
        ...
        Camera.Size size = choosePreviewSize(parms, encWidth, encHeight);
    ...
    private static Camera.Size choosePreviewSize(Camera.Parameters parms, int w, int h) {
